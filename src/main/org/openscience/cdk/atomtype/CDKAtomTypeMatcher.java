    ...
    private IAtomType perceiveSelenium(IAtomContainer atomContainer, IAtom atom) throws CDKException {
        ...
        if (atom.getFormalCharge() != CDKConstants.UNSET
            ...
            if (atomContainer.getConnectedAtomsCount(atom) == 3) {
                IAtomType type = getAtomType("Se.4");
                if (isAcceptable(atom, atomContainer, type)) {
                    return type; // Fix59 //
                }
            }
            if (atomContainer.getConnectedAtomsCount(atom) == 4) {
                IAtomType type = getAtomType("Se.6");
                if (isAcceptable(atom, atomContainer, type)) {
                    return type; // Fix60 //
                }
            }
        } else if (atomContainer.getConnectedAtomsCount(atom) == 0
                && (atom.getFormalCharge() != CDKConstants.UNSET && atom.getFormalCharge() == 4)) {

            IAtomType type = getAtomType("Se.4plus");
            if (isAcceptable(atom, atomContainer, type)) {
                return type; // Fix108 //
            }
        } else if ((atom.getFormalCharge() != CDKConstants.UNSET && atom.getFormalCharge() == 1) && atomContainer.getConnectedAtomsCount(atom) == 3) {
    ...
    private IAtomType perceiveNitrogenRadicals(IAtomContainer atomContainer, IAtom atom) throws CDKException {
        if (atomContainer.getConnectedBondsCount(atom) >= 1
            ...
            if (atom.getFormalCharge() != CDKConstants.UNSET
                ...
                IAtomType type = getAtomType("N.2minus.planar");
                if (isAcceptable(atom, atomContainer, type)) {
                    return type;
                } // Fix43 //
            } else if (atom.getFormalCharge() == CDKConstants.UNSET
        ...
        if (hasOneSingleElectron(atomContainer, atom)) {
            ...
            if (isRingAtom(atom, atomContainer) && bothNeighborsAreSp2(atom, atomContainer)) {
                if (countAttachedDoubleBonds(atomContainer, atom) == 2) {
                    IAtomType type = getAtomType("S.sp2");
                    if (isAcceptable(atom, atomContainer, type)) {
                        return type;
                    }//fix102
                } else {
                ...
                IAtomType type = getAtomType("S.2.2");
                if (isAcceptable(atom, atomContainer, type)) {
            ...
            if (atomContainer.getConnectedBondsList(atom).get(0).getOrder() == CDKConstants.BONDORDER_DOUBLE) {
                IAtomType type = getAtomType("S.2.1");
                if (isAcceptable(atom, atomContainer, type)) {
                    return type;
                }//changes made here//
            ...
            IAtomType type = getAtomType("S.4");
            if (isAcceptable(atom, atomContainer, type)) {
                return type;
            }  // Fix51 //
            ...
            int doubleBondedAtoms = countAttachedDoubleBonds(atomContainer, atom);
            if (doubleBondedAtoms == 1) {
                IAtomType type = getAtomType("S.5");
                if (isAcceptable(atom, atomContainer, type)) {
                    return type;
                } // Fix50.1 //
            }
    ...
    private IAtomType perceivePhosphors(IAtomContainer atomContainer, IAtom atom)
        ...
        if (countSingleElectrons(atomContainer, atom) == 3) {
            ...
            if (maxBondOrder == CDKConstants.BONDORDER_DOUBLE) {
                ...
                if (atom.getAtomicNumber() != null && atom.getAtomicNumber() == 15) {
                    IAtomType type = getAtomType("P.32");
                    if (isAcceptable(atom, atomContainer, type)) {
                        return type;
                    }
            ...
            if (!hasLonePair(atom) && atom.getFormalCharge().intValue() == 0) {
                IAtomType type = getAtomType("P.ate");
                if (isAcceptable(atom, atomContainer, type)) {
                    return type;
                }//Fix 93
            ...
            } else if (atom.getAtomicNumber() != null && atom.getAtomicNumber() == 15) {
                IAtomType type = getAtomType("P.32");
                if (isAcceptable(atom, atomContainer, type)) {
                    return type;
                }
            ...
            if (hasOneSingleElectron(atomContainer, atom)) {
                ...
                if (doubleBondCount == 2) {
                    ...
                    if (isAcceptable(atom, atomContainer, type)) {
                        ...
                        && atom.getFormalCharge() == 0) {
                    IAtomType type = getAtomType("I.3.sp3d2");
                    if (isAcceptable(atom, atomContainer, type)) {
                        return type;
                    }
                }
    ...
    private IAtomType perceiveCommonSalts(IAtomContainer atomContainer, IAtom atom) throws CDKException {
        ...
        if ("Na".equals(atom.getSymbol())) {
            ...
            if (hasOneSingleElectron(atomContainer, atom)) {
                ...
                if (neighbors == 4) {
                    IAtomType type = getAtomType("Mg.4.neutral");
                    if (isAcceptable(atom, atomContainer, type)) {
                        return type;// Fix45 //
                    }
                }
                if (neighbors == 2) {
                    IAtomType type = getAtomType("Mg.2.neutral");
                    if (isAcceptable(atom, atomContainer, type)) {
                        return type;// Fix46 //
                    }
                }
                if (neighbors == 1) {
                    IAtomType type = getAtomType("Mg.1.neutral");
                    if (isAcceptable(atom, atomContainer, type)) {
                        return type;// Fix47 //
                    }
                }
        ...
        if ("Hg".equals(atom.getSymbol())) {
            ...
            if (hasOneSingleElectron(atomContainer, atom)) {
                ...
                if (isAcceptable(atom, atomContainer, type)) {
                    ...
                    && atom.getFormalCharge() == +4) && atom.getFormalNeighbourCount() <= 4) {
                IAtomType type = getAtomType("As.4plus");
                if (isAcceptable(atom, atomContainer, type)) {
                    return type; // Fix90 //
                }
            }
